...
$stmt = mysqli_prepare($conn, "SELECT `password`,user_type FROM user_details WHERE mobileNumber = ?");
...
$hashedPassword = password_hash($password, PASSWORD_BCRYPT);
    ...
    $user_type='3';
    // Prepare the query
    $stmt = mysqli_prepare($conn, "INSERT INTO `user_details` (`user_type`,`mobileNumber`,`password` ) VALUES (?, ?, ?)");
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        // echo json_encode(["msg" => "Signup successful!","status"=>'success']);
            ...
            $sql = "SELECT * FROM `user_details` where `mobileNumber`='$mobileNumber' ";
            ...
            if ($result){
                ...
                http_response_code(200);
                echo json_encode(['status' => 'success', 'msg' => 'sign up successful','user_type' =>$user_type ,'userData'=>$userData ]);
