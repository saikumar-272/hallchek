...
if (empty($hall_id) || empty($slot_id) || empty($date) || empty($statustoUpdate) ) {
...
if ($result) {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO `bookedslotstable`( `hall_id`, `slot_id`, `date`) VALUES ( ?, ?, ?)");
    ...
    mysqli_stmt_bind_param($stmt, "sis", $hall_id, $slot_id, $date );
